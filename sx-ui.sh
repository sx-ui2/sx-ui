...
allow_port_via_iptables() {
    ...
    if [[ ${panel_opened} -eq 1 ]]; then
        LOGI "已通过 iptables 增量放行面板端口 ${port}/tcp"
        if [[ "${ssh_port}" != "${port}" && ${ssh_opened} -eq 1 ]]; then
            LOGI "已通过 iptables 保留 SSH 端口 ${ssh_port}/tcp"
        fi
        if ! persist_iptables_rules; then
            LOGW "iptables 规则已临时生效，但未检测到持久化工具，重启后可能失效。"
        fi
        return 0
    fi
...
allow_port_in_firewall() {
	...
	if [[ ${opened} -eq 0 ]]; then
		LOGW "未检测到可用的 firewalld/UFW/iptables，已跳过自动放行面板端口。"
	fi
}
